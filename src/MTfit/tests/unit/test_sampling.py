...
class SampleTestCase(unittest.TestCase):
    ...
    def test_append(self):
        moment_tensors = np.array([[2], [1], [2], [1], [2], [1]])
        ln_pdf = LnPDF(np.array([[1], [2]]))
        self.Sample.append(moment_tensors, ln_pdf, 1)
        self.assertTrue((self.Sample.moment_tensors[:, 0:1] == np.array([[2], [1], [2], [1], [2], [1]])).all())
        self.assertEqual(self.Sample.n, 1)
        self.assertEqual(self.Sample.ln_pdf[0, 0], 1.)
        moment_tensors = np.array([[2, 3], [1, 5], [2, 1], [1, 1], [2, 2], [1, 1]])
        ln_pdf = LnPDF(np.array([[1, 0], [2, 1]]))
        self.Sample.append(moment_tensors, ln_pdf, 2)
        self.assertTrue((self.Sample.moment_tensors[:, 0:3] == np.array([[2, 2, 3], [1, 1, 5], [2, 2, 1], [1, 1, 1], [2, 2, 2], [1, 1, 1]])).all())
        moment_tensors = np.array([[2], [1], [2], [1], [2], [1]])
        ln_pdf = LnPDF(np.array([[0], [0]]))
        self.Sample.append(moment_tensors, ln_pdf, 1)
        self.assertTrue((self.Sample.moment_tensors[:, 0:3] == np.array([[2, 2, 3], [1, 1, 5], [2, 2, 1], [1, 1, 1], [2, 2, 2], [1, 1, 1]])).all())
        self.Sample.append(np.array([[], [], [], [], [], []]), np.array([[]]), 1)
        self.assertTrue((self.Sample.moment_tensors[:, 0:3] == np.array([[2, 2, 3], [1, 1, 5], [2, 2, 1], [1, 1, 1], [2, 2, 2], [1, 1, 1]])).all())
        self.assertEqual(self.Sample.n, 5)
        # test multiple events
        self.tearDown()
        self.Sample = Sample(initial_sample_size=1, number_events=2)
        moment_tensors = [np.array([[2], [1], [2], [1], [2], [1]]), np.array([[2], [1], [2], [1], [2], [1]])]
        ln_pdf = LnPDF(np.array([[1]]))
        self.Sample.append(moment_tensors, ln_pdf, 1)
        self.assertTrue((self.Sample.moment_tensors[:, 0:1] == np.array([[2], [1], [2], [1], [2], [1], [2], [1], [2], [1], [2], [1]])).all())
        ...
        ln_pdf = LnPDF(np.array([[1], [2]]))
        self.Sample.append(moment_tensors, ln_pdf, 1)
        self.assertTrue((self.Sample.moment_tensors[:, 0:4] == np.array([[2, 2, 3, 2], [1, 1, 5, 1], [2, 2, 1, 2], [1, 1, 1, 1], [2, 2, 2, 2], [1, 1, 1, 1]])).all())

    def test_output(self):
        self.test_append()
        self.assertTrue((self.Sample.output()[0]['moment_tensor_space'] == np.array([[2, 2, 3, 2], [1, 1, 5, 1], [2, 2, 1, 2], [1, 1, 1, 1], [2, 2, 2, 2], [1, 1, 1, 1]])).all())
        output_ln_pdf = np.sum(np.exp(np.array([[1., 1., 0., 1.], [2., 2., 1., 2.]])), 0)
        output_ln_pdf /= np.sum(output_ln_pdf)
        self.assertAlmostEqual(self.Sample.output()[0]['probability'].flat[0], output_ln_pdf[0])
        self.assertAlmostEqual(self.Sample.output()[0]['probability'].flat[1], output_ln_pdf[1])
        self.assertAlmostEqual(self.Sample.output()[0]['probability'].flat[2], output_ln_pdf[2])
        self.assertAlmostEqual(self.Sample.output()[0]['probability'].flat[3], output_ln_pdf[3])
        self.assertEqual(self.Sample.output()[0]['dV'], 1)
        output_ln_pdf = np.sum(np.exp(np.array([[1., 1., 0., 1.], [2., 2., 1., 2.]])), 0)
        self.assertAlmostEqual(self.Sample.output(False)[0]['probability'].flat[0], output_ln_pdf[0])
        self.assertAlmostEqual(self.Sample.output(False)[0]['probability'].flat[1], output_ln_pdf[1])
        self.assertAlmostEqual(self.Sample.output(False)[0]['probability'].flat[2], output_ln_pdf[2])
        self.assertAlmostEqual(self.Sample.output(False)[0]['probability'].flat[3], output_ln_pdf[3])
        self.assertEqual(self.Sample.output(False)[0]['dV'], 1)
        self.tearDown()
        self.Sample = Sample(initial_sample_size=1, number_events=2)
        moment_tensors = [np.array([[2], [1], [2], [1], [2], [1]]), np.array([[3], [1], [2], [1], [2], [1]])]
        ln_pdf = LnPDF(np.array([[1]]))
        self.Sample.append(moment_tensors, ln_pdf, 1)
        self.assertTrue((self.Sample.moment_tensors[:, 0:1] == np.array([[2], [1], [2], [1], [2], [1], [3], [1], [2], [1], [2], [1]])).all())
        self.assertTrue((self.Sample.output()[0]['moment_tensor_space_1'] == np.array([[2], [1], [2], [1], [2], [1]])).all())
        self.assertTrue((self.Sample.output()[0]['moment_tensor_space_2'] == np.array([[3], [1], [2], [1], [2], [1]])).all())
...
class FileSampleTestCase(unittest.TestCase):
    ...
    def test_append(self):
        self.assertEqual(self.FileSample.fname, 'test_in_progress.mat')
        self.assertEqual(self.FileSample.n, 0)
        self.FileSample.append(np.array([[1], [2], [1], [2], [1], [1]]), np.array([[1]]), 10)
        ...
        self.FileSample.append(np.array([[1], [2], [1], [2], [1], [1]]), np.array([[1]]), 20)
        ...
        self.FileSample.append(np.array([[1], [2], [1], [2], [1], [1]]), np.array([[-np.inf]]), 20)
        self.assertEqual(self.FileSample.n, 50)
        x = loadmat(self.FileSample.fname)
        self.assertEqual(sorted(x.keys()), ['LnPDF_1', 'LnPDF_2', 'MTSpace_1', 'MTSpace_2', 'i', 'n', 'non_zero_samples'])
        ...
        self.assertEqual(self.FileSample.fname, 'test_in_progress.mat')
        self.assertEqual(self.FileSample.n, 0)
        self.FileSample.append(np.array([[1], [2], [1], [2], [1], [1]]), np.array([[1]]), 10, [{'mu': np.array([[1, 2], [2, 1]]), 'sigma': np.array([[0, 0.1], [0.1, 0]])}])
        self.assertEqual(self.FileSample.n, 10)
        self.assertTrue(os.path.exists(self.FileSample.fname))
        from hdf5storage import loadmat
        x = loadmat(self.FileSample.fname)
        ...
        self.assertEqual(x['n'], 10)
        self.FileSample.append(np.array([[1], [2], [1], [2], [1], [1]]), np.array([[1]]), 20, [{'mu': np.array([[1, 2], [2, 1]]), 'sigma': np.array([[0, 0.1], [0.1, 0]])}])
        self.assertEqual(self.FileSample.n, 30)
        self.assertTrue(os.path.exists(self.FileSample.fname))
        x = loadmat(self.FileSample.fname)
        self.assertEqual(sorted(x.keys()), ['LnPDF_1', 'LnPDF_2', 'MTSpace_1', 'MTSpace_2', 'i', 'n', 'non_zero_samples', 'scale_factor_1', 'scale_factor_2'])
        self.assertEqual(x['n'], 30)
        self.FileSample.append(np.array([[1], [2], [1], [2], [1], [1]]), np.array([[-np.inf]]), 20, [{'mu': np.array([[1, 2], [2, 1]]), 'sigma': np.array([[0, 0.1], [0.1, 0]])}])
        self.assertEqual(self.FileSample.n, 50)
        x = loadmat(self.FileSample.fname)
        self.assertEqual(sorted(x.keys()), ['LnPDF_1', 'LnPDF_2', 'MTSpace_1', 'MTSpace_2', 'i', 'n', 'non_zero_samples', 'scale_factor_1', 'scale_factor_2'])
        self.assertEqual(x['n'], 50)
