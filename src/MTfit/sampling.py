...
class Sample:
    ...
    def __init__(self, initial_sample_size: int = 100000, number_events: int = 1, prior=_6sphere_prior) -> None:
        """
        ...
        """
        self.moment_tensors = np.zeros((number_events*6, initial_sample_size))
        self.n = 0
        self.ln_pdf = LnPDF()
    ...
    def append(self, moment_tensors: np.ndarray, ln_pdf, n: int, scale_factor=False, extensions_scale_factor=False) -> None:
        """
        Appends new samples to the Sample object, extending the MT array if the limit is reached.

        Args
            moment_tensors: numpy array of moment tensors to append.
            ln_pdf: numpy array or PDF object containing ln_pdf samples corresponding to the moment tensors in moment_tensors (must be same length).
        ...
        if isinstance(ln_pdf, (float, int, np.float64, np.float32)) and ((isinstance(moment_tensors, np.ndarray) and moment_tensors.shape[1] != 1) or (isinstance(moment_tensors, list) and max([mt.shape[1] for mt in moment_tensors]) != 1)):
    ...
    def output(self, normalise: bool = True, convert: bool = False, n_samples: int = 0, discard: int = 10000, mcmc: bool = False):
        """
        ...
        output_string += f'Sample Max Probability: {ln_pdf.max()}\n'
        ...
        output_string += f'Sample Max Probability MT:\n{max_p_mt}\n\n'
        # Check if discarding samples
        if discard and n_samples:
            output_string += f'Discarding samples less than 1/{discard*n_samples} of the maximum likelihood value as negligeable\n\n'
        # Check if there are samples
        if len(ln_pdf):
            # Get non_zero samples
            non_zero = ln_pdf.nonzero(discard=discard, n_samples=n_samples)
            if discard and n_samples:
                output_string += f'After discard, {non_zero.shape[0]} samples remain\n\n'
            if len(ln_pdf.shape) > 1:
            ...
            if self.number_events > 1:
                ...
                for i in range(self.number_events):
                    output[f'moment_tensor_space_{i+1}'] = moment_tensors[6*i:6*(i+1), non_zero]
                    if convert:
                        # convert results
                        output.update(_convert(output[f'moment_tensor_space_{i+1}'], i+1))
            else:
            ...
            try:
                if not mcmc:
                    ...
                    output_string += f"PDF Kullback-Leibler Divergence Estimate (Dkl): {output['dkl']}\n\n"
            except Exception:
                pass
...
class FileSample(Sample):
    ...
    def __init__(self, fname: str = 'MTfit_run', file_safe: bool = True, *args, **kwargs) -> None:
        """
        ...
        self.pdf = LnPDF()
        self.moment_tensor = np.zeros((6, 0))
    ...
    def append(self, moment_tensors: np.ndarray, ln_pdf, n: int, scale_factor=False, extensions_scale_factor=False) -> None:
        ...
        if isinstance(moment_tensors, list) and isinstance(moment_tensors[0], np.ndarray):
            moment_tensors = np.array(moment_tensors)
            moment_tensors = moment_tensors.reshape(moment_tensors.shape[0]*moment_tensors.shape[1], moment_tensors.shape[2])
        if isinstance(ln_pdf, (float, int, np.float64, np.float32)) and moment_tensors.shape[1] != 1:
            raise ValueError('Moment Tensor shape must be 6x1 when using a float ln_pdf')
        elif not isinstance(ln_pdf, (float, int, np.float64, np.float32)) and moment_tensors.shape[1] != ln_pdf.shape[1]:
            raise ValueError('Moment Tensor shape[2] and ln_pdf shape[2] must be the same')
        if isinstance(ln_pdf, (np.ndarray, float, int, np.float64, np.float32)):
        ...
        if isinstance(extensions_scale_factor, dict) and len(extensions_scale_factor):
            # Scale factor dict containing mu and s
            for key in extensions_scale_factor.keys():
                extensions_scale_factor[key] = list(extensions_scale_factor[key][ln_pdf.nonzero()])
        if self.file_safe:
        ...
        if moment_tensors.shape[0]*moment_tensors.shape[1] > 0:  # Check there are samples
            if len(ln_pdf.nonzero()) != moment_tensors.shape[1]:
                raise ValueError('Moment Tensor shape[2] and ln_pdf shape[2] must be the same')
            self.savemat(self.fname, {f'MTSpace_{self._i}': moment_tensors}, appendmat=not self.file_safe, store_python_metadata=True)
            if not isinstance(scale_factor, bool):
                # dict with p mu and s
                self.savemat(self.fname, {f'scale_factor_{self._i}': list(scale_factor)}, appendmat=not self.file_safe, store_python_metadata=True)
            if isinstance(extensions_scale_factor, dict) and len(extensions_scale_factor):
                # dict with p mu and s
                self.savemat(self.fname, {f'extensions_scale_factor_{self._i}': extensions_scale_factor}, appendmat=not self.file_safe, store_python_metadata=True)
            self.savemat(self.fname, {f'LnPDF_{self._i}': ln_pdf[:, ln_pdf.nonzero()]}, appendmat=not self.file_safe, store_python_metadata=True)
    ...
    def output(self, *args, **kwargs):
        ...
        self.moment_tensors = np.zeros((self.number_events*6, self.non_zero_samples))
        ...
        for key in [key for key in data.keys() if 'MTSpace' in key]:
            i = key.split('_')[-1]
            self.ln_pdf.append(LnPDF(data[f'LnPDF_{i}']))
            moment_tensors = data[f'MTSpace_{i}']
            self.moment_tensors[:, _x: _x+moment_tensors.shape[1]] = moment_tensors
            if isinstance(self.scale_factor, list):
                self.scale_factor.append(data[f'scale_factor_{i}'])
...
def ln_bayesian_evidence(output: dict, n_samples: int, prior=_6sphere_prior) -> float:
    """
...
def _convert(moment_tensors: np.ndarray, i: int | None = None) -> dict:
    """
    Converts the moment tensor to Tape parameters, Hudson u,v and strike dip rake pairs.

    Args
        moment_tensors: moment tensor array.
